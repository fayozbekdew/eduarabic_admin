      ...
      <div className="messenger">
        <div className="messenger-chat">
          {selectedChat && selectedChat.messages.length > 0 ? (
            selectedChat.messages.map((item: MessageType, id: number) => {
              ...
              return (
                <div className={`msg ${item.sender === currentUser?.id ? "right" : "left"}-msg`} key={id}>
                  {item?.name ? <div className="msg-img" /> : <Image src={`/src/assets/images/${participators?.image}`} className="img-30 h-auto" alt="user" />}
            ...
            <Image className="img-fluid w-100 h-auto" src={`/src/assets/images/start-conversion.jpg`} alt="start conversion"/>
          )}
        </div>
      </div>
      <SendMessage />
