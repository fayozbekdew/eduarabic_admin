    ...
    <Gallery withCaption>
      {galleryImagePath.map((item, index) => (
          <figure key={index} className="col-xl-3 col-sm-6 box-col-25">
            <Item original={`/src/assets/images/${item}`} width="1024" height="768" caption="is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy.">
              {({ ref, open }) => (
                <Link to={Href} onClick={open}>
                  <img className="img-thumbnail" ref={ref as React.MutableRefObject<HTMLImageElement>} src={`/src/assets/images/${item}`} alt="thumbnail"/>
