          ...
          <td className="px-0">
            <div className="d-flex align-items-center">
              <div className="flex-shrink-0">
                <Image src={`/src/assets/images/dashboard-2/svg-icon/${data.img}`} alt="icons"/>
              </div>
            ...
            <UL className="simple-list flex-row">
              {data.images.map((item, i) => (
                <LI className="d-inline-block" key={i}>
                  {item !== "" ? (
                    <Image className="img-30 rounded-circle" src={`/src/assets/images/dashboard-2/user/${item}`} alt="user" />
                  ) : (
